...
var window # iframe 창
var add_todo_func = JavaScriptBridge.create_callback(Callable(self, 'add_todo'))
var remove_todo_func = JavaScriptBridge.create_callback(Callable(self, 'remove_todo'))
...
func _ready():
	...
	dir = DirAccess.open('user://todo/')
	if dir:
		dir.list_dir_begin()
		var ls = dir.get_next()
		while ls:
			var file:= FileAccess.open('user://todo/%s/info.todo' % ls, FileAccess.READ)
			if file: # 혹시 완료된 일인지도 검토
...
func add_todo(args):
	...
	if json is Dictionary:
		...
		if json.has('done') and json.done:
			if check_exist:
				var inst:= have_done.instantiate()
				inst.name = 'have_done'
		...
		match(json.importance):
			'0': # 메모
				new_todo = ele_0.instantiate()
				new_todo.line_color = Color('#888888bb')
				new_todo.normal_color = Color('#88888888')
				new_todo.alert_color = Color('#00bbbb88')
				new_todo.lerp_start_from = .8
			'1': # 기억해야함
				new_todo = ele_1.instantiate()
				new_todo.line_color = Color('#dddd0cbb')
				new_todo.normal_color = Color('#88888888')
				new_todo.alert_color = Color('#dddd0c88')
				new_todo.lerp_start_from = .5
			'2': # 중요함
				new_todo = ele_2.instantiate()
				new_todo.line_color = Color('#880000bb')
				new_todo.normal_color = Color('#dddd0c88')
				new_todo.alert_color = Color('#88000088')
		...
		randomize()
		$Todos/Area2D/GenerateHere.position = Vector2(max_dist / 2 - (50 + randf() * 75), 0)
		$Todos/Area2D.rotation = deg_to_rad(randf() * 720 - 360)
...
func _process(_delta):
	current_time = int(Time.get_unix_time_from_system() * 1000)
	window_size = $Todos.size
