...
func _ready():
	get_tree().connect("files_dropped", Callable(self, 'load_package_debug'))
...
func load_package_debug(files:PackedStringArray, scr):
	var target:String
	for file in files:
		if file.find('.pck') + 1:
			target = file
...
func load_package(act_name:String):
	...
	if not is_loaded: # 없으면 다운받기
		...
		if not $CenterContainer/ColorRect.is_connected("gui_input", Callable(self, '_on_Label_gui_input')):
			$CenterContainer/ColorRect.connect("gui_input", Callable(self, '_on_Label_gui_input'))
	else: # 패키지를 가지고 있는 경우
...
func start_download_pck():
	...
	req.connect("request_completed", Callable(self, '_on_HTTPRequest_request_completed'))
